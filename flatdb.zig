...
/// The cut mode controls which side of delimiter should be cut out from the
/// buffer.
pub const CutMode = enum {
    /// Cut everything before the delimiter.
    before,
    /// Cut everything after the delimiter.
    after,
};
...
/// The scan mode controls where to start scanning the buffer for a delimiter.
pub const ScanMode = enum {
    /// Scan the buffer from the start.
    start,
    /// Scan the buffer from the end.
    end,
};
...
pub fn CutBufferOptions(comptime T: type) type {
    return struct {
        /// Set of values to scan for, indicating where to cut the buffer.
        delims: []const T,
        /// Which side of delimiter to cut out.
        cut_mode: CutMode,
        /// Which side of buffer to start scanning for delimiter.
        scan_mode: ScanMode,
        /// Whether delimiter should also be cut out.
        include_delim: bool,
    };
}
...
/// Cut out spans from values generated by a buffer iterator.
pub fn CutBufferIterator(
    comptime Iterator: type,
    comptime T: type,
    comptime options: CutBufferOptions(T),
) type {
    const delims = options.delims;
    const cut_mode = options.cut_mode;
    const scan_mode = options.scan_mode;
    const include_delim = options.include_delim;

    return struct {
        inner: Iterator,

        /// Initialize inner iterator buffer.
        pub fn init(buffer: []const u8) @This() {
            return .{ .inner = Iterator.init(buffer) };
        }

        /// Return the next value after cutting out unwanted data.  Return
        /// null when no more values are available.
        pub fn next(this: *@This()) ?[]const T {
            if (this.inner.next()) |value| {
                var start: usize = 0;
                var end: usize = value.len;

                if (this.scan(value)) |pos| {
                    switch (cut_mode) {
                        .after => end = pos + if (include_delim) 0 else 1,
                        .before => start = pos + if (include_delim) 1 else 0,
                    }
                }

                return if (start < end) value[start..end] else value[0..];
            } else {
                return null;
            }
        }

        /// Scan value for a delimiter.  Return null if no delimiter is found.
        fn scan(this: @This(), value: []const T) ?usize {
            _ = this; // TODO: make this static?

            return switch (scan_mode) {
                .start => mem.indexOfAny(u8, value, delims),
                .end => mem.lastIndexOfAny(u8, value, delims),
            };
        }
    };
}
...
/// Filter values generated by a buffer iterator.
pub fn FilterBufferIterator(
    comptime Iterator: type,
    comptime T: type,
    comptime filter: fn ([]const T) bool,
) type {
    return struct {
        inner: Iterator,
        ...
        /// Return the next value that matches the filter.  Return null when no
...
test "filtering" {
    // test filter to remove empty lines
    const local = struct {
        pub fn filter(value: []const u8) bool {
            return value.len > 0;
        }
    };

    // filtered iterator over simple text lines
    const FilterIterator = FilterBufferIterator(DelimitedBufferIterator(u8, .{
        .delims = &.{'\n'},
        .delimit_mode = .separator,
    }), u8, local.filter);
    ...
    var it = FilterIterator.init(buffer);
...
test "cutting buffers" {
    // cut out everything after the first delimiter
    const CutIterator = CutBufferIterator(DelimitedBufferIterator(u8, .{
        .delims = &.{'\n'},
        .delimit_mode = .terminator,
        .collapse = true,
    }), u8, .{
    ...
    var it = CutIterator.init(buffer);

    try std.testing.expectEqualStrings("name", it.next().?);
    try std.testing.expectEqualStrings("apple", it.next().?);
    try std.testing.expectEqualStrings("banana", it.next().?);
    try std.testing.expectEqualStrings("carrot", it.next().?);
    try std.testing.expectEqual(null, it.next());
}
